...
type InvertedIndex interface {
	...
	GetGroupingContext(tagKeyIDs []tag.KeyID, seriesIDs *roaring.Bitmap) (series.GroupingContext, error)
...
func (index *invertedIndex) GetGroupingContext(
	...
	scannerMap := make(map[tag.KeyID][]series.GroupingScanner)
	for _, tagKeyID := range tagKeyIDs {
...
func (index *invertedIndex) GetGroupingContexts(
	tagKeyIDs []tag.KeyID,
	tagValueIDs []*roaring.Bitmap,
) (series.GroupingContext, error) {
	// get kv store snapshot
	invertedSnapshot := index.invertedFamily.GetSnapshot()
	defer invertedSnapshot.Close()

	// get grouping scanners by tag key
	scanners, err := index.getGroupingScanners2(tagKeyIDs, tagValueIDs, invertedSnapshot)
	if err != nil {
		return nil, err
	}
	return query.NewGroupContext2(tagValueIDs, scanners), nil
}
...
// getGroupingScanners returns the grouping scanner list for tag key, need match series ids
func (index *invertedIndex) getGroupingScanners2(
	tagKeyIDs []tag.KeyID,
	tagValueIDs []*roaring.Bitmap,
	invertedSnapshot version.Snapshot,
) ([][]*roaring.Bitmap, error) {
	result := make([][]*roaring.Bitmap, len(tagKeyIDs))
	for idx, tagKeyID := range tagKeyIDs {
		tagValueIDsForTag := tagValueIDs[idx]
		seriesIDs := make([]*roaring.Bitmap, tagValueIDsForTag.GetCardinality())
		result[idx] = seriesIDs
		// read data from mem
		index.loadSeriesIDsInMem(tagKeyID, func(tagIndex TagIndex) {
			// get grouping scanner in memory, no err throw
			tagIndex.GetSeriesIDsArrayByTagValueIDs(tagValueIDs[idx], seriesIDs)
		})
		// read data from kv store
		// try to get tag key id from kv store
		readers, err := invertedSnapshot.FindReaders(uint32(tagKeyID))
		if err != nil {
			// find table.Reader err, return it
			return nil, err
		}
		...
		if len(readers) > 0 {
			...
			err := reader.GetSeriesIDsArrayByTagValueIDs(tagKeyID, tagValueIDsForTag, seriesIDs)
			if err != nil {
				return nil, err
			}
		}
